class BeerClubsController < ApplicationController
  ...
  def show
    ...
    @confirmed_members = Membership.confirmed.where(beer_club_id: params[:id]).to_a
    @applicants = Membership.applicant.where(beer_club_id: params[:id]).to_a

    @current_membership = Membership.where(user_id: current_user.id, beer_club_id: @beer_club.id).first
  end
